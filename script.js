...
const provinceMessages = {
  Ontario:
    "Ontario is Canada's most populous province, home to Toronto and Ottawa.",
  Quebec:
    "Quebec is known for its French heritage, with Montreal and Quebec City as major centers.",
  "British Columbia":
    "British Columbia features Vancouver and stunning natural landscapes like the Rockies.",
};
...
buttonContainer.addEventListener("click", function (event) {
  if (event.target.classList.contains("delegatedButton")) {
    ...
    // Update the message
    provinceInfo.textContent =
      provinceMessages[provinceName] || "No information available.";

    // Change the background color of th box
    provinceInfo.style.backgroundColor =
      provinceColors[provinceName] || "#f8f9fa"; // Default fallback
  }
});
